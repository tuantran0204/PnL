...
def calculate_metrics(data, funded_cac_increase, new_customer_increases2024, new_customer_increases2025, new_customer_increases2026, new_customer_increases2027, new_customer_increases2028, active_rate, funding_rate):
    ...
    data['EBIT'] =  data['Gross Profit'] - ((data['Funded Customer'] * data['Funded CAC']) - (data['Staff Cost'] +  data['Opex'] + data['Retaining'] + data['Selling Cost']) / 1000)
...
fig_test_chart = go.Figure()
    create_column_chart(fig_test_chart, processed_data['Year'], processed_data['Opex'], 'Funded Customers (Unit: Thousand)')
